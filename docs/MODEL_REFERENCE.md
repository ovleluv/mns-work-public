...
Model behaviour notes accumulated per version, v0.2 through v49.11, moved here unchanged from the
README. Later entries refine earlier ones; where they disagree the later entry and the code win.
...
- Added a [BML generation guide](BML_GUIDE.md#part-1--authoring-checklist) for authoring side-specific plans from scenario IDs, map bounds, supported tasks, perceived Tracks, conditions, phases, and directives. It includes a loadable example and a headless validation workflow.
